...
public class Maze implements IMaze {
	...
	public Collection<Tile> getUnexploredTiles() {
		List<Tile> unexploredTiles = new ArrayList<Tile>();
		for (Tile tile : getTiles()) {
			if (!tile.isExplored()) {
	...
	public Collection<Tile> getSeesawBarcodeTiles() {
		Collection<Tile> seesawTiles = new HashSet<>();
		for (Tile tile : tiles.values())
			if (tile.hasBarcode()
					&& TeamTreasureTrekBarcodeMapping.isSeesawBarcode(tile
							.getBarcode()))
				seesawTiles.add(tile);
		return seesawTiles;
	}
