...
public class RectangularEnvelope implements Envelope {
	...
	public RectangularEnvelope(double height, double width)
			throws IllegalArgumentException {
		if (height == 0 || width == 0) {
			throw new IllegalArgumentException(
					"The given length and width may not be equal to zero.");
		}
	...
	private Point2D getClosestPointOnSegment(Point2D startOfSegment,
			Point2D endOfSegment, Point2D target) {
	...
	private double getSegmentParameter(Point2D startOfSegment,
			Point2D endOfSegment, Point2D target) {
		double tx = (target.getX() - startOfSegment.getX())
				* (endOfSegment.getX() - startOfSegment.getX());
		double ty = (target.getY() - startOfSegment.getY())
				* (endOfSegment.getY() - startOfSegment.getY());
		double squared2Norm = startOfSegment.distanceSq(endOfSegment);
		return (tx + ty) / (squared2Norm);
	}

	private Point2D[] getCornerPoints(Pose pose) {
		float center_x = pose.getX();
		float center_y = pose.getY();
		float angle = pose.getHeading();

		Point2D[] cps = new Point2D.Double[4];
		// cps[i] is a neighbour of cps[i+1] and cps[i-1] with the index % 4
		// (e.g.: i % 4,(i+1) % 4,(i-1) % 4)
		cps[0] = rotatePoint(new Point2D.Double((center_x + getHeight() / 2),
				(center_y + getWidth() / 2)), angle);
		cps[1] = rotatePoint(new Point2D.Double((center_x - getHeight() / 2),
				(center_y + getWidth() / 2)), angle);
		cps[2] = rotatePoint(new Point2D.Double((center_x - getHeight() / 2),
				(center_y - getWidth() / 2)), angle);
		cps[3] = rotatePoint(new Point2D.Double((center_x + getHeight() / 2),
				(center_y - getWidth() / 2)), angle);
		return cps;
	}
	...
	public Point2D[] getDiscretization(Pose pose, int depth)
		...
		for (int j = 0; j < 4; j++) {
			for (int i = 0; i < depth; i++) {
				tps[(int) (j * d + i)] = getPointAtSegmentParameter(cps[j],
						cps[(j + 1) % 4], d * i);
			}
		}
		return tps;
	...
	private Point2D getPointAtSegmentParameter(Point2D startOfSegment,
			Point2D endOfSegment, double segmentParameter) {
		...
		double c_x = startOfSegment.getX() + segmentParameter
				* (endOfSegment.getX() - startOfSegment.getX());
		double c_y = startOfSegment.getY() + segmentParameter
				* (endOfSegment.getY() - startOfSegment.getY());
